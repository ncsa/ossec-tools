...
SYSCHECK_CONTROL=/var/ossec/bin/syscheck-control
ROOTKIT_CONTROL=/var/ossec/bin/rootkit-control
...
This script should be run on the OSSEC server.
     ...
     Options:
        ...
        -S              Syscheck last scan check
        -R              Rootkit last scan check
...
while getopts "ha:c:ls:v:w:SR" OPTION
do
  case $OPTION in
    h)
      usage;;
...
if [ $CHECK_SYSCHECK -eq 1 ]; then


fi

if [ $CHECK_ROOTCHECK -eq 1 ]; then
fi
